...
class OneLogin_Saml2_Response(object):
    ...
    def is_valid(self, request_data, request_id=None, raise_exceptions=False):
        ...
        try:
            ...
            has_signed_assertion = (
                "{%s}Assertion" % OneLogin_Saml2_Constants.NS_SAML in signed_elements
            )
            ...
            if self._settings.is_strict():
                ...
                if security["wantNameIdEncrypted"]:
                    encrypted_nameid_nodes = self._query_assertion(
                ...
                if (
                    security["failOnAuthnContextMismatch"]
                    and requested_authn_contexts
                    and requested_authn_contexts is not True
                ):
                ...
                attribute_statement_nodes = self._query_assertion(
                    "/saml:AttributeStatement"
                )
                if (
                    security.get("wantAttributeStatement", True)
                    and not attribute_statement_nodes
                ):
                    raise OneLogin_Saml2_ValidationError(
                ...
                encrypted_attributes_nodes = self._query_assertion(
                ...
                if (
                    session_expiration
                    and session_expiration <= OneLogin_Saml2_Utils.now()
                ):
                    raise OneLogin_Saml2_ValidationError(
                ...
                subject_confirmation_nodes = self._query_assertion(
                ...
                for scn in subject_confirmation_nodes:
                    ...
                    if sc_data is None:
                        ...
                        if nooa:
                            self.valid_scd_not_on_or_after = (
                                OneLogin_Saml2_Utils.parse_SAML_to_time(nooa)
                            )
            ...
            if not signed_elements or (
                ...
                multicerts = None
                if (
                    "x509certMulti" in idp_data
                    and "signing" in idp_data["x509certMulti"]
                    and idp_data["x509certMulti"]["signing"]
                ):
                ...
                document_check_assertion = (
                    self.decrypted_document if self.encrypted else self.document
                )
    ...
    def check_status(self):
        ...
        if code and code != OneLogin_Saml2_Constants.STATUS_SUCCESS:
            ...
            status_exception_msg = (
                "The status code of the Response was not Success, was %s"
                % printable_code
            )
    ...
    def get_authn_contexts(self):
        ...
        :rtype: list
        """
        authn_context_nodes = self._query_assertion(
    ...
    def get_nameid_data(self):
        ...
        encrypted_id_data_nodes = self._query_assertion(
            "/saml:Subject/saml:EncryptedID/xenc:EncryptedData"
        )
        ...
        if nameid is None:
            ...
            if (
                is_strict
                and want_nameid
                and not OneLogin_Saml2_XML.element_text(nameid)
            ):
                raise OneLogin_Saml2_ValidationError(
    ...
    def get_session_not_on_or_after(self):
        ...
        authn_statement_nodes = self._query_assertion(
    ...
    def _get_attributes(self, attr_name):
        allow_duplicates = self._settings.get_security_data().get(
            "allowRepeatAttributeName", False
        )
        attributes = {}
        attribute_nodes = self._query_assertion(
    ...
    def validate_num_assertions(self):
        ...
        if self.encrypted:
            assertion_nodes = OneLogin_Saml2_XML.query(
                self.decrypted_document, "//saml:Assertion"
            )
    ...
    def process_signed_elements(self):
        ...
        for sign_node in sign_nodes:
            ...
            if reject_deprecated_alg:
                sig_method_node = OneLogin_Saml2_XML.query(
                    sign_node, ".//ds:SignatureMethod"
                )
                if sig_method_node:
                ...
                dig_method_node = OneLogin_Saml2_XML.query(
                    sign_node, ".//ds:DigestMethod"
                )
                if dig_method_node:
        ...
        if signed_elements:
            if not self.validate_signed_elements(
                signed_elements, raise_exceptions=True
            ):
                raise OneLogin_Saml2_ValidationError(
    ...
    def validate_signed_elements(self, signed_elements):
        ...
        if (
            (
                response_tag in signed_elements
                and signed_elements.count(response_tag) > 1
            )
            or (
                assertion_tag in signed_elements
                and signed_elements.count(assertion_tag) > 1
            )
            or (
                response_tag not in signed_elements
                and assertion_tag not in signed_elements
            )
        ):
            return False
        ...
        if assertion_tag in signed_elements:
            expected_signature_nodes = self._query(
    ...
    def validate_timestamps(self):
        ...
        for conditions_node in conditions_nodes:
            ...
            if (
                nb_attr
                and OneLogin_Saml2_Utils.parse_SAML_to_time(nb_attr)
            ...
            if (
                nooa_attr
                and OneLogin_Saml2_Utils.parse_SAML_to_time(nooa_attr)
                + OneLogin_Saml2_Constants.ALLOWED_CLOCK_DRIFT
                <= OneLogin_Saml2_Utils.now()
            ):
                raise OneLogin_Saml2_ValidationError(
